...
def print_surface_analysis(aggregator):
    ...
    log("\n" + "-"*50)
    log("2D EDGE SURFACE: DROPS (Price Declined)")
    log("-"*50)
    log("Format: n_samples | uncond_edge | fill_rate | edge_after_fill (SE)")
    
    # Header row
    header = f"{'Interval':<15}"
    for thresh in MOVE_THRESHOLDS:
        header += f" | >={thresh*100:.0f}%".center(30)
    log(header)
    log("-" * len(header))
    ...
    for interval_label in [x[2] for x in INTERVAL_PAIRS]:
        ...
        for thresh in MOVE_THRESHOLDS:
            cell = summary['surface'][interval_label]['drop'].get(thresh, {})
            if cell.get('status') == 'insufficient_data':
                row += f" | n={cell.get('n_samples', 0):>3}".center(30)
            elif cell.get('status') == 'ok':
                n = cell['n_samples']
                uncond = cell['unconditional_edge_bps']
                fill_rate = cell['fill_rate']
                edge_after = cell.get('edge_after_fill_bps')
                se = cell.get('se_edge_after_fill', 0)
                
                if edge_after is not None:
                    row += f" | {n:>4} | {uncond:+.0f} | {fill_rate:.0%} | {edge_after:+.0f}({se:.0f})".ljust(30)
                else:
                    row += f" | {n:>4} | {uncond:+.0f} | {fill_rate:.0%} | --".ljust(30)
            else:
                row += " | --".center(30)
        log(row)
    
    log("\n" + "-"*50)
    log("2D EDGE SURFACE: RISES (Price Increased)")
    log("-"*50)
    log("Format: n_samples | uncond_edge | fill_rate | edge_after_fill (SE)")
    ...
    for interval_label in [x[2] for x in INTERVAL_PAIRS]:
        row = f"{interval_label:<15}"
        for thresh in MOVE_THRESHOLDS:
            cell = summary['surface'][interval_label]['rise'].get(thresh, {})
            if cell.get('status') == 'insufficient_data':
                row += f" | n={cell.get('n_samples', 0):>3}".center(30)
            elif cell.get('status') == 'ok':
                n = cell['n_samples']
    ...
    with open(report_path, 'w') as f:
        ...
        for interval_label in summary.get('surface', {}):
            for direction in ['drop', 'rise']:
                for thresh, cell in summary['surface'][interval_label].get(direction, {}).items():
                    ...
                    entry = (interval_label, thresh, edge, cell)
                    
                    if direction == 'drop':
                        if best_drop is None or edge > best_drop[2]:
                            best_drop = entry
                    else:
                        if best_rise is None or edge < best_rise[2]:  # Lower is worse for buys
                            best_rise = entry
        
        if best_drop:
            f.write(f"\nBest DROPS (underreaction):\n")
            f.write(f"  Interval: {best_drop[0]}, Threshold: >={best_drop[1]*100:.0f}%\n")
            f.write(f"  Edge after fill: {best_drop[2]:+.0f} bps\n")
            f.write(f"  Sample size: {best_drop[3]['n_samples']}\n")
        
        if best_rise:
            f.write(f"\nMost negative RISES (overreaction):\n")
            f.write(f"  Interval: {best_rise[0]}, Threshold: >={best_rise[1]*100:.0f}%\n")
            f.write(f"  Edge after fill: {best_rise[2]:+.0f} bps\n")
            f.write(f"  Sample size: {best_rise[3]['n_samples']}\n")
