...
OBJECTIVE:
  Validate that the formalized Strategy B rules extract consistent positive edge
  before layering on position sizing, temporal sequencing, and portfolio metrics.
  This is a "unit test" for the strategy logic.

STRATEGY B RULES (Updated from Phase 4E Delay Analysis):
  - Prob buckets: sub_51 ONLY (51-75% is "dead zone" with negative edge)
  - Windows: 48h→24h, 24h→12h (batch-viable, edge retained at 15-30min delay)
  - Threshold: ≥10% (balances edge vs capacity, 5% was noise)
...
KEY FINDINGS FROM DELAY ANALYSIS:
  - sub_51 at 48h→24h, 10% threshold: +185 bps at 0s, +65 bps at 30min
  - Edge half-life ~15 minutes (batch job viable)
  - 51-75% buckets show NEGATIVE edge even at instant reaction - excluded
  - Close-to-resolution windows (6h→4h) collapse within 5 minutes - excluded
...
USAGE:
  ...
  python phase5_strategy_b_confirmatory_v2.py

  # Diagnostic mode
  python phase5_strategy_b_confirmatory_v2.py --diagnostic

  # Custom parameters
  python phase5_strategy_b_confirmatory_v2.py --min-threshold 0.15

  # Force rebuild with reaction delay
  python phase5_strategy_b_confirmatory_v2.py --rebuild --reaction-delay 300
"""
...
STRATEGY_B_DEFAULTS = {
    # sub_51 ONLY - the 51-75% range is a "dead zone" with negative edge
    'prob_buckets': ['sub_51'],
    
    # Batch-viable windows only (edge retained at 15-30min delay)
    # Excluded: 6h→4h, 8h→4h, 9h→6h (edge collapses within 5min)
    # Excluded: 12h→6h (mixed results, information structure issues)
    'intervals': ['48h_to_24h', '24h_to_12h'],
    
    # 10% minimum (5% was too noisy in delay analysis)
    'min_threshold': 0.10,
...
def generate_strategy_b_report(filtered_stats, unfiltered_stats, params, output_dir, 
                                quintile_thresholds, diagnostic=False, reaction_delay=0):
    """
    Generate comprehensive text report for Strategy B confirmatory analysis.
    """
    suffix = '_DIAGNOSTIC' if diagnostic else ''
    report_path = os.path.join(output_dir, f'strategyB_confirmatory_v2_report{suffix}_{TIMESTAMP}.txt')
    
    log(f"\nGenerating report: {report_path}")
    
    with open(report_path, 'w') as f:
        f.write("="*80 + "\n")
        f.write("STRATEGY B CONFIRMATORY ANALYSIS v2.0: LONGSHOT FADING (BATCH-VIABLE)\n")
        f.write("="*80 + "\n\n")
        ...
        f.write("2. PROBABILITY BUCKET UPDATE:\n")
        f.write("   Removed 51-75% 'dead zone' - these buckets show NEGATIVE edge\n")
        f.write("   even at instant reaction across all time windows.\n\n")
        
        f.write("3. WINDOW UPDATE:\n")
        ...
        f.write("4. THRESHOLD UPDATE:\n")
        f.write("   Increased minimum threshold to 10% (5% was noise in delay analysis).\n\n")
        ...
        # ---------------------------------------------------------------------
        # STRATEGY B RESULTS (Primary Output)
        # ---------------------------------------------------------------------
        f.write("="*80 + "\n")
        f.write("STRATEGY B RESULTS (CORRECTED FILL PRICES)\n")
        f.write("="*80 + "\n\n")
        ...
        if unfiltered_stats and filtered_stats:
            ...
            f.write(f"Strategy B filter applied:\n")
        ...
        f.write("Based on Phase 4E delay analysis for sub_51, 48h→24h, 10% threshold:\n\n")
        f.write("  Delay       Edge (bps)    Retention\n")
        f.write("  ---------   ----------    ---------\n")
        f.write("  0s (inst)   +185          100%\n")
        f.write("  5min        +131          71%\n")
        f.write("  15min       +85           46%\n")
        f.write("  30min       +65           35%\n")
        f.write("  1hr         +17           9%\n")
        f.write("  2hr         -27           negative\n")
        f.write("\n")
        f.write("RECOMMENDATION: Target <15min reaction for meaningful edge capture.\n")
        f.write("               Batch jobs at 15-30min intervals are viable but marginal.\n")
        f.write("\n")
        ...
        if filtered_stats:
            ...
            # Strategy B note: different risk profile than A
            f.write("Strategy B Risk Profile Notes:\n")
            f.write("  - Lower win rate expected (betting on longshots)\n")
            f.write("  - Losses are typically smaller per trade (buying cheap options)\n")
            f.write("  - Occasional large wins from longshot payoffs\n")
            f.write("\n")
            ...
            if skew > 200:
                f.write("✓  Positive skew detected (mean > median).\n")
                f.write("    Occasional large wins lift average returns — expected for longshots.\n")
            
            f.write("\n")
            
            # Longshot characterization
            f.write("Longshot Payoff Structure:\n")
            f.write(f"  Win rate:                {filtered_stats['win_rate']*100:.1f}%\n")
            if filtered_stats['win_rate'] > 0:
                ...
                implied_odds = 1 / filtered_stats['win_rate'] if filtered_stats['win_rate'] > 0 else 0
                f.write(f"  Implied odds (1/WR):     {implied_odds:.1f}x\n")
        f.write("\n")
        ...
        f.write("   Returns are computed from the probability (price) at entry.\n\n")
        ...
        if filtered_stats:
            ...
            f.write("\n")
            f.write("Note: Strategy B (longshots) typically has higher capacity than Strategy A\n")
            f.write("      due to the broader sub_51 bucket containing more markets.\n")
            f.write("      However, v2.0 restricts to batch-viable windows which may reduce\n")
            f.write("      opportunity count vs the original broader window selection.\n")
        f.write("\n")
        ...
        if filtered_stats and filtered_stats['mean'] > 0:
            f.write("✓  Strategy B shows POSITIVE expected edge.\n")
            ...
            f.write("✗  Strategy B shows NEGATIVE or ZERO expected edge.\n")
            f.write(f"   Mean return: {filtered_stats['mean']:+.1f} bps per trade\n")
            f.write("\n")
...
def rebuild_volume_data(sample_files=None, diagnostic=False, reaction_delay=0):
    ...
    if diagnostic:
        returns_dir = os.path.join(OUTPUT_DIR, f'strategyB_rebuild_v2_DIAGNOSTIC{delay_suffix}_{TIMESTAMP}')
    else:
        returns_dir = os.path.join(OUTPUT_DIR, f'strategyB_rebuild_v2{delay_suffix}_{TIMESTAMP}')
...
def main():
    parser = argparse.ArgumentParser(
        description='Strategy B Confirmatory Analysis v2.0: Longshot Fading (Batch-Viable)',
...
Examples:
  ...
  python phase5_strategy_b_confirmatory_v2.py --prob-buckets sub_51 75_90

  # Use specific baseline directory
  python phase5_strategy_b_confirmatory_v2.py --baseline-dir /path/to/volume_data

  # Force rebuild with reaction delay
  python phase5_strategy_b_confirmatory_v2.py --rebuild --reaction-delay 300 --sample 1000
...
VERSION 2.0 CHANGES:
  ...
  - Updated defaults: sub_51 only, 48h/24h windows, 10%+ threshold
  - Added --reaction-delay parameter for rebuild mode
  - Removed 51-75% "dead zone" from defaults
        """
    )
    ...
    parser.add_argument('--prob-buckets', nargs='+', 
                        ...
                        help=f"Probability buckets (default: {STRATEGY_B_DEFAULTS['prob_buckets']})")
    parser.add_argument('--intervals', nargs='+',
                        default=STRATEGY_B_DEFAULTS['intervals'],
                        help=f"Interval labels (default: {STRATEGY_B_DEFAULTS['intervals']})")
    parser.add_argument('--min-threshold', type=float,
                        default=STRATEGY_B_DEFAULTS['min_threshold'],
                        help=f"Minimum threshold (default: {STRATEGY_B_DEFAULTS['min_threshold']})")
    parser.add_argument('--terciles', nargs='+',
                        default=STRATEGY_B_DEFAULTS['terciles'],
                        help=f"Tercile labels (default: {STRATEGY_B_DEFAULTS['terciles']})")
    parser.add_argument('--volume-quintiles', nargs='+',
                        default=STRATEGY_B_DEFAULTS['volume_quintiles'],
                        help=f"Volume quintiles (default: {STRATEGY_B_DEFAULTS['volume_quintiles']})")
    ...
    log("="*70)
    log("STRATEGY B CONFIRMATORY ANALYSIS v2.0")
    log("="*70)
    ...
    # -------------------------------------------------------------------------
    # STEP 5: Apply Strategy B filter
    # -------------------------------------------------------------------------
    ...
    report_path = generate_strategy_b_report(
