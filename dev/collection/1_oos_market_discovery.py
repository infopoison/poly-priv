...
# CLOB API HELPER
# ==========================================
def fetch_market_end_date(condition_id, max_retries=3):
    """Fetch actual market end_date from CLOB API. Returns Unix timestamp or None."""
    ...
    for attempt in range(max_retries):
        try:
            ...
            if resp.status_code == 404:
                ...
                log(f"  CLOB rate limited, waiting 30s...", "WARN")
                time.sleep(30)
                continue
...
def fetch_resolved_markets(after_timestamp, before_timestamp, output_dir):
    ...
    # Step 2/3: Validate end_date via CLOB and filter
    # Redemption timestamp != market end date. We must check CLOB.
    log(f"\nStep 2/3: Validating market end dates via CLOB...")
    log(f"(Filtering to markets that ENDED after {datetime.fromtimestamp(after_timestamp, tz=timezone.utc).strftime('%Y-%m-%d')})")
    
    after_dt = datetime.fromtimestamp(after_timestamp, tz=timezone.utc)
    before_dt = datetime.fromtimestamp(before_timestamp, tz=timezone.utc) if before_timestamp else None
    
    clob_checkpoint_file = os.path.join(output_dir, "oos_clob_validation_checkpoint.json")
    
    # Load CLOB checkpoint if exists
    validated_conditions = {}  # condition_id -> end_date timestamp (or None if invalid)
    
    if os.path.exists(clob_checkpoint_file):
        try:
            with open(clob_checkpoint_file, 'r') as f:
                validated_conditions = json.load(f)
            log(f"✓ Resuming CLOB validation: {len(validated_conditions):,} already checked")
        except:
            pass
    
    remaining_conditions = [c for c in condition_times.keys() if c not in validated_conditions]
    log(f"Conditions to validate: {len(remaining_conditions):,}")
    log(f"Estimated time: {len(remaining_conditions) * 1.0 / 60:.0f} minutes")
    ...
    for i, cond_id in enumerate(remaining_conditions):
        if (i + 1) % 100 == 0:
            elapsed = time.time() - clob_start
            rate = (i + 1) / elapsed if elapsed > 0 else 1
            eta_min = (len(remaining_conditions) - i - 1) / rate / 60
            log(f"  [{i+1}/{len(remaining_conditions)}] In: {in_window}, Out: {out_window}, Err: {errors}, ETA: {eta_min:.0f}m")
        
        # Query CLOB
        end_ts = fetch_market_end_date(cond_id)
        
        if end_ts is None:
            validated_conditions[cond_id] = None
            errors += 1
        else:
            validated_conditions[cond_id] = end_ts
            end_dt = datetime.fromtimestamp(end_ts, tz=timezone.utc)
            if end_dt >= after_dt and (before_dt is None or end_dt <= before_dt):
                in_window += 1
            else:
                out_window += 1
        ...
        if (i + 1) % 500 == 0:
            with open(clob_checkpoint_file, 'w') as f:
                json.dump(validated_conditions, f)
    
    # Final checkpoint
    with open(clob_checkpoint_file, 'w') as f:
        json.dump(validated_conditions, f)
    
    # Filter condition_times to only valid ones
    filtered_condition_times = {}
    for cond_id, redemption_ts in condition_times.items():
        end_ts = validated_conditions.get(cond_id)
        if end_ts is None:
            continue
        end_dt = datetime.fromtimestamp(end_ts, tz=timezone.utc)
        if end_dt >= after_dt and (before_dt is None or end_dt <= before_dt):
            filtered_condition_times[cond_id] = end_ts  # Use actual end_ts, not redemption_ts
    
    log(f"\n✓ CLOB validation complete:")
    log(f"  In window:     {len(filtered_condition_times):,}")
    log(f"  Out of window: {out_window:,}")
    log(f"  Errors:        {errors:,}")
    
    # Replace condition_times with filtered version
    condition_times = filtered_condition_times
    
    # Clean up checkpoint
    if os.path.exists(clob_checkpoint_file):
        os.remove(clob_checkpoint_file)
